...
| **小动物管理** | 代管动物（列表/网格、列设置）、笼位可视化、操作申请工作流（申领需管理员审核） |
| **科研助理** | 工作台每日待办（API 同步）、进度收集 / PPT / 报销 / 物资预约入口 |
| **预约审批** | 待审批通知、批准 / 拒绝、原子时段冲突检测 |
| **权限与审计** | 多角色、仅管理员开户赋权、按负责人范围的审批日志、全量操作日志 |
...
| 角色 | 说明 |
|------|------|
| `super_admin` | 总管理员，开通账号、分配身份、全局数据 |
| `instrument_manager` | 仪器负责人，管理仪器与相关预约审批 |
| `animal_manager` | 区域负责人，管理区域内资源与相关审批 |
| `research_assistant` | 科研助理 |
| `user` | 普通用户，预约与代管申请 |

支持同一账号多角色。账号须由总管理员在「用户管理」中开通，不可自行注册选角。
...
| `GET/POST` | `/api/todos` | RA 每日待办列表 / 创建 |
| `PATCH/DELETE` | `/api/todos/[id]` | 勾选完成 / 删除待办 |
